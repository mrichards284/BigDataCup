...
library(magick)
  ...
  nhl_logo = readPNG('/Users/marcrichards/Desktop/Big Data Cup/Images/Otters.png')
...
sample_loc <- data.frame(x = c(23,35,48,61),
           y = c(0,0,0,0),
           `Goal Probability` = c(0.03,0.07,0.15,0.35))

rink + geom_point(data = data.frame(x = -60,y = 25),aes(x=x,y=y),size = 7) +
  geom_point(data = sample_loc,aes(x=x,y=y,color = Goal.Probability),size = 7) +
  annotate("segment",x = -60,y = 25,xend = 23,yend = 0,linetype = "dotted",color = "black") +
  annotate(geom = "text",x = -50,y = 31,label = "Jamie Drysdale",size = 5.5) +
  annotate(geom = "text",x = 25,y = 6,label = "Maxim Golod",size = 5.5) +
  ...
  labs(color='Goal Probability') +
  theme(legend.position = "bottom",
        legend.title = element_text(size = 16))


rink +
  stat_density_2d(data = scouting_passes_xHA %>% 
                    filter(Player == "Jamie Drysdale") %>%
                    mutate(X.Coordinate = X.Coordinate - 100,Y.Coordinate = Y.Coordinate - 42.5 ),
                  aes(x = X.Coordinate,y = Y.Coordinate,fill = ..level..),geom = "polygon",colour = "white")
...
rink +
  geom_density_2d(data = scouting_passes_xHA %>% 
                    filter(Player == "Jamie Drysdale") %>%
                    mutate(X.Coordinate = X.Coordinate - 100,Y.Coordinate = Y.Coordinate - 42.5 ),
                  aes(x = X.Coordinate,y = Y.Coordinate,color = comp_prob))

rink +
  geom_hex(data = scouting_passes_xHA %>% 
             filter(Player == "Jamie Drysdale") %>%
             mutate(X.Coordinate = X.Coordinate - 100,Y.Coordinate = Y.Coordinate - 42.5 ),
...
rink +
  geom_point(data = scouting_passes_wgoals %>% 
               filter(Team == "Erie Otters") %>%
               mutate(X.Coordinate = X.Coordinate - 100,Y.Coordinate = Y.Coordinate - 42.5 ),
             aes(x = X.Coordinate,y = Y.Coordinate)) +
  stat_density_2d(aes(fill = comp_prob),geom = "polygon",colour = "white")

rink +
  geom_point(data = scouting_passes_wgoals %>% 
               filter(Player == "Jamie Drysdale") %>%
               mutate(X.Coordinate = X.Coordinate.2 - 100,Y.Coordinate = Y.Coordinate.2 - 42.5 ),
             aes(x = X.Coordinate,y = Y.Coordinate,color = factor(goal))) +
  scale_color_fivethirtyeight() +
  theme_fivethirtyeight() 


rink +
  geom_point(data = scouting_passes_xHA %>% 
               filter(Player == "Jamie Drysdale") %>%
               mutate(X.Coordinate = X.Coordinate - 100,Y.Coordinate = Y.Coordinate - 42.5 ),
             aes(x = X.Coordinate,y = Y.Coordinate,color = xHA)) +
  #scale_color_fivethirtyeight() +
  theme_fivethirtyeight() 
...
rink + geom_point(data = scouting_passes[1,] %>% 
                    mutate(X.Coordinate = X.Coordinate - 100,Y.Coordinate = Y.Coordinate - 42.5 ),aes(x = X.Coordinate,y = Y.Coordinate))
